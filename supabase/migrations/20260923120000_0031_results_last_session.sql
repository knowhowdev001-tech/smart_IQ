-- The results header, after a restart.
--
-- It shows the session just finished, which the app holds in memory only --
-- so opening results from home, or reopening the app, showed 0% while every
-- figure below it was correct. The summary now carries the most recent
-- completed session and the header falls back to it.

create or replace function public.get_results_summary(
...
begin
  ...
  select jsonb_build_object(
    ...
    -- The most recent finished session, whatever the window: the header
    -- calls it the last session, and scoping it to the range would blank it
    -- out for exactly the reader it is there for.
