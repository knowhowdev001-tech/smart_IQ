-- "All time" means every day since the user started, not the last eight.
--
-- The axis was three different shapes: one day, seven days, or the eight
-- most recent days with practice. That last one closed up the gaps, so a
-- month of sporadic practice read as a solid run. Now all three are the same
-- shape -- a run of calendar days ending today -- and only the start moves:
-- today, six days back, or the user's first completed session.
--
-- The client scrolls the chart when there are more bars than fit, so the
-- axis is deliberately uncapped.
